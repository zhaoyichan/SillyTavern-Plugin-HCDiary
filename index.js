...
// 在 _cdDoInit 中被注册为 GENERATION_AFTER_COMMANDS 的监听器
let _cdInjectionRegistered = false;

async function cdOnBeforeGeneration(eventData) {
  // CHAT_COMPLETION_PROMPT_READY 事件：eventData = { chat: [], dryRun: bool, ... }
  try {
    ...
    chat.push(sysMsg);
    cdLog('cdOnBeforeGeneration: 已注入日记到 prompt chat', text.length);
  } catch (e) {
    cdWarn('cdOnBeforeGeneration 失败', e);
  }
}

// 旧的 cdInjectDiaryToPrompt 保留，仅作标记（实际注入由 cdOnBeforeGeneration 完成）
async function cdInjectDiaryToPrompt() {
  // 只做日志，实际注入交给 cdOnBeforeGeneration
  cdLog('cdInjectDiaryToPrompt: 标记完成，等待下次生成时注入');
}
...
async function cdRunDiary({ manual = false, silent = false } = {}) {
  ...
  cdBusy = true;
  try {
    if (typeof toastr !== "undefined") toastr.info(`开始写日记 (${windowFloors.length} 个新楼层)...`);
    ...
    if (diaryOk || relOk || archiveOk) {
      ...
      // 注入日记到下次AI对话上下文
      if (diaryOk || archiveOk) await cdInjectDiaryToPrompt();
      {
        const tips = [];
...
async function cdOnMessageReceived() {
  ...
  // 调试：显示事件已触发
  if (typeof toastr !== 'undefined') toastr.info('[角色日记] 消息事件触发，检查楼层...');
  
  const data = await cdGetData();
  const newFloors = await cdGetNewFloors(data);
  const newCount = newFloors.length;
  
  cdLog('自动触发检查: 新楼层数', newCount, '间隔', s.interval);
  
  if (newCount > 0) {
    // 有新楼层就显示
    if (typeof toastr !== 'undefined') toastr.info(`[角色日记] 检测到 ${newCount} 个新AI楼层 (阈值${s.interval})`);
  }
...
async function _cdDoInit() {
  ...
  try {
    ...
    if (!es || !et) {
      ...
      cdLog('[init] 注册ST事件...');
      // 注册 CHAT_COMPLETION_PROMPT_READY：在 prompt 构建后、发AI前注入日记
      if (et.CHAT_COMPLETION_PROMPT_READY && !_cdInjectionRegistered) {
        es.on(et.CHAT_COMPLETION_PROMPT_READY, cdOnBeforeGeneration);
        _cdInjectionRegistered = true;
        cdLog('[init] 注册 CHAT_COMPLETION_PROMPT_READY 注入');
      }
      if (_cdListeners.char) es.removeListener?.(et.CHARACTER_MESSAGE_RENDERED, _cdListeners.char);
      _cdListeners.char = () => {
        try {
          if (typeof toastr !== 'undefined') toastr.info('[CD] AI消息事件到达');
          cdOnMessageReceived();
        } catch(e) {
          cdWarn('cdOnMessageReceived 异常', e);
          if (typeof toastr !== 'undefined') toastr.error('[角色日记] 消息处理出错: ' + (e && e.message));
        }
      };
      es.on(et.CHARACTER_MESSAGE_RENDERED, _cdListeners.char);
      // 双重保险：TauriTavern 可能用 MESSAGE_RECEIVED
      if (et.MESSAGE_RECEIVED) {
        es.on(et.MESSAGE_RECEIVED, _cdListeners.char);
        cdLog('[init] 同时监听 MESSAGE_RECEIVED');
      }
      ...
      <div class="cd-sheet">
        <div class="cd-header">
          ...
          <div class="cd-header-actions">
            <button class="cd-header-btn" id="cd-btn-fullscreen" title="全屏"><i class="fa-solid fa-maximize"></i></button>
            <button class="cd-header-btn" id="cd-btn-settings" title="设置"><i class="fa-solid fa-sliders"></i></button>
            <button class="cd-header-btn cd-close" id="cd-btn-close" title="关闭"><i class="fa-solid fa-xmark"></i></button>
          </div>
        </div>

        <div class="cd-toolbar">
          <button class="cd-tb-btn cd-tb-active" id="cd-tb-browse" data-mode="browse"><i class="fa-solid fa-list"></i> 浏览
          </button>
          <button class="cd-tb-btn" id="cd-tb-graph" data-mode="graph"><i class="fa-solid fa-diagram-project"></i> 关系
          </button>
          <button class="cd-tb-btn" id="cd-tb-archive" data-mode="archive"><i class="fa-solid fa-archive"></i> 剧情档案
          </button>
          <button class="cd-tb-btn" id="cd-tb-write" data-mode="write"><i class="fa-solid fa-feather-pointed"></i> 写日记
          </button>
          <button class="cd-tb-btn" id="cd-tb-backfill" data-mode="backfill"><i class="fa-solid fa-clock-rotate-left"></i> 补写
          </button>
          <button class="cd-tb-btn cd-tb-danger" id="cd-tb-clear" data-mode="clear"><i class="fa-solid fa-trash"></i> 清空
          </button>
        </div>
        ...
        ${list.slice().reverse().map(e => `
          <div class="cd-entry" data-floor="${e.message_id || ''}">
            <div class="cd-entry-head">
              ...
              ${e.mood ? `<span class="cd-entry-mood">${cdMoodEmoji(e.mood)} ${escapeHtml(e.mood)}</span>` : ''}
...
async function cdRenderGraph() {
  ...
  // 找核心人物（被提到最多的/出场最多的当根节点）
  // 简单策略：日记数量最多的角色当根
  let rootName = names[0];
  let maxCount = 0;
  for (const [name, list] of Object.entries(data.diaries)) {
    if (list.length > maxCount) { maxCount = list.length; rootName = name; }
  }
  
  // 建树
  // 第一层是根节点，第二层是跟根有关系的人，第三层是跟第二层有关系但不在前面的人
  const used = new Set([rootName]);
  const tree = { name: rootName, children: [] };
  
  // 构建子节点
  const children = [];
  const childRels = rels[rootName] || {};
  for (const [to, r] of Object.entries(childRels)) {
    if (!used.has(to)) {
      used.add(to);
      children.push({ name: to, rel: r, children: [] });
    }
  }
  // 其他没关系的节点也加到第二层
  for (const n of names) {
    if (!used.has(n)) {
      used.add(n);
      children.push({ name: n, rel: null, children: [] });
    }
  }
  // 第三层：子节点的子节点
  for (const child of children) {
    const grandRels = rels[child.name] || {};
    for (const [to, r] of Object.entries(grandRels)) {
      if (!used.has(to)) {
        used.add(to);
        child.children.push({ name: to, rel: r });
      }
    }
  }
  ...
  // 渲染树
  let html = '<div class="cd-tree-wrap">' + renderTree(tree, rels) + '</div>';
  
  // 下方文字列表
  html += '<div class="cd-graph-text" style="margin-top:12px">';
  let hasAnyRels = false;
  for (const from of names) {
    const targets = rels[from] || {};
    const entries = Object.entries(targets);
    if (!entries.length) continue;
    hasAnyRels = true;
    html += '<div class="cd-graph-node"><strong class="cd-graph-from">' + escapeHtml(from) + '</strong>';
    for (const [to, r] of entries) {
      const a = r.attitude || 'neutral';
      const icon = a === 'positive' ? '<i class="fa-solid fa-face-smile" style="color:#5a9;"></i>' : a === 'negative' ? '<i class="fa-solid fa-face-angry" style="color:#c55;"></i>' : '<i class="fa-solid fa-face-meh" style="color:#a98;"></i>';
      html += '<div class="cd-graph-rel">' + icon + ' → <span class="cd-graph-to">' + escapeHtml(to) + '</span>';
      if (r.type) html += '<span class="cd-graph-type">' + escapeHtml(r.type) + '</span>';
      if (r.note) html += '<span class="cd-graph-note">' + escapeHtml(r.note) + '</span>';
      html += '</div>';
    }
    html += '</div>';
  }
  if (!hasAnyRels) html += '<div class="cd-empty"><p>暂无关系数据</p></div>';
  html += '</div>';
...
/** 横竖连线关系图 */
function renderTree(node, rels, depth) {
  if (depth === undefined) depth = 0;
  
  if (depth === 0) {
    let html = '<div class="cd-lines-wrap">';
    // 左侧竖线容器
    html += '<div class="cd-lines-left">';
    html += '<div class="cd-lines-node cd-lines-root">' + escapeHtml(node.name) + '</div>';
    html += '</div>';
    
    if (node.children && node.children.length) {
      html += '<div class="cd-lines-right">';
      html += '<div class="cd-lines-vline"></div>';
      for (let i = 0; i < node.children.length; i++) {
        const child = node.children[i];
        const isLast = i === node.children.length - 1;
        html += '<div class="cd-lines-row">';
        // 连线部分：横线 + 关系标签
        html += '<div class="cd-lines-conn">';
        html += '<div class="cd-lines-hline"></div>';
        if (child.rel) {
          const a = child.rel.attitude || 'neutral';
          const icon = a === 'positive' ? '<i class="fa-solid fa-face-smile" style="color:#5a9;"></i>' : a === 'negative' ? '<i class="fa-solid fa-face-angry" style="color:#c55;"></i>' : '';
          html += '<span class="cd-lines-label">' + icon + ' ' + escapeHtml(child.rel.type || '关系') + '</span>';
        }
        html += '</div>';
        // 角色方框
        html += '<div class="cd-lines-node">' + escapeHtml(child.name) + '</div>';
        html += '</div>';
      }
      html += '</div>';
    }
  ...
  return '<div class="cd-lines-node">' + escapeHtml(node.name) + '</div>';
}
...
async function cdRenderArchive() {
  ...
  if (empty) {
    $('#cd-content').html(`<div class="cd-empty"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"/></svg><p>暂无剧情档案</p><p class="cd-empty-sub">写日记时将自动生成</p></div>`);
    return;
  }
  const html = `
    <div class="cd-archive">
      ...
      <div class="cd-archive-section">
        <h3 class="cd-archive-label"><i class="fa-solid fa-route"></i> 主线</h3>
        <div class="cd-archive-text">${escapeHtml(arc.mainline).replace(/\n/g, '<br>')}</div>
      </div>` : ''}
      ${arc.sideline ? `
      <div class="cd-archive-section">
        <h3 class="cd-archive-label"><i class="fa-solid fa-code-branch"></i> 支线</h3>
        <div class="cd-archive-text">${escapeHtml(arc.sideline).replace(/\n/g, '<br>')}</div>
      </div>` : ''}
      ${arc.states ? `
      <div class="cd-archive-section">
        <h3 class="cd-archive-label"><i class="fa-solid fa-chart-simple"></i> 状态</h3>
        <div class="cd-archive-text">${escapeHtml(arc.states).replace(/\n/g, '<br>')}</div>
      </div>` : ''}
      ${arc.unresolved ? `
      <div class="cd-archive-section">
        <h3 class="cd-archive-label"><i class="fa-solid fa-triangle-exclamation"></i> 未解决</h3>
...
function cdRenderWrite() {
  $('#cd-content').html(`
    <div class="cd-empty">
      ...
      <p>立即触发一次日记生成</p>
      <button class="cd-btn-primary" id="cd-do-write">立即写日记</button>
    </div>`);
...
async function cdRenderSettings() {
  ...
  const panel = $('#cd-settings-panel');
  panel.html(`
    <h2 class="cd-settings-h2"><i class="fa-solid fa-gear"></i> 设置</h2>
    ...
    <div id="cd-custom-api" style="display:${s.source === 'tavern' ? 'none' : 'block'};">
      ...
      <button class="cd-btn-secondary" id="cd-btn-fetch-models">拉取模型列表</button>
    </div>
